...
3. **성능**
- 사용자가 도서 상태(대출 가능 여부)를 도서관 시스템에서 실시간으로 확인할 수 있어야 한다. (CQRS)
- 사용자가 자신이 대출한 책과, 현재 누적 연체료를 실시간으로 확인할 수 있어야 한다. (CQRS)
- 대출 및 반납 상태가 변경될 때마다 사용자에게 이메일 또는 메시지로 알림을 전송할 수 있어야 한다. (Event driven)
